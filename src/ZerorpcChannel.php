...
use Leafpoda\Zerorpc\Channel\ZerorpcChannelException;

class ZerorpcChannel
{
    ...
    public function invoke(ZerorpcEvent $event)
    {
        if (microtime(true) > $this->expire) throw new ZerorpcChannelException('Timed out');

        if ($event->name === '_zpc_hb') {
            // Send heartbeat response
            return $this->socket->send(new ZerorpcEvent($this->envelope, $this->createHeader(), '_zpc_hb'));
        }
    ...
    public function send($name, array $args = null)
    {
        ...
        $event = new ZerorpcEvent($this->envelope, $this->createHeader(), $name, $args);
